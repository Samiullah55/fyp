...
async function imageSubmit() {
  ...
  const s3Url = await axios.get("http://13.48.194.207:5005/s3Url");
  console.log(s3Url.data.url);
  console.log(file);
  console.log(file.type);
  const response = await axios.put(s3Url.data.url, file, {
    headers: {
      "Content-Type": file.type,
    },
  });
  console.log(response.data);
  const imgUrl = s3Url.data.url.split("?")[0];
  console.log(imgUrl);
        ...
        {submitted ? (
          ...
          >
            ...
            <div className="btn-block btn">
              <button type="submit" onClick={imageSubmit}>
                Submit
              </button>
            </div>
          </form>
